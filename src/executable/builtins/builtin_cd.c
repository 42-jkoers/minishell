...
void* builtin_cd_main(const t_executable* command)
{
	...
	{
		char* dir = *(char**)list_index(&command->args, 1);
		bool success = working_directory_set(path_join(working_directory_get(), dir));
		if (!success)
			return ft_strjoin_va(3, "bash: cd: ", dir, ": No such file or directory\n");
	}
	return NULL;
}
