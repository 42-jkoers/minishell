...
static void log_env_data(char** variable)
{
	...
	char* eq = ft_strchr(*variable, '=');
	if (!eq)
		ft_putstr_fd(*variable, STDOUT_FILENO);
	else
	{
		write(STDOUT_FILENO, *variable, (eq - *variable) / sizeof(char) + sizeof(char));
...
static void add_arg(char** variable)
{
	...
	if (!eq)
		return;
	char* name = ft_substr(*variable, 0, eq - *variable);
...
void	*builtin_export_main(const t_executable *command)
{
	if (command->args.count == 1)
		return ((void*)1);
	list_foreach_range(&command->args, (t_range){.start = 1, .end = command->args.count}, (t_foreach_value)add_arg);
	return ((void*)0);
}
